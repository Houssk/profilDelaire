...
C:
cd C:\Program Files (x86)\EasyPHP-DevServer-14.1VC9\data\localweb\projects\Planification3D_v2\img
...
for %%X in (%dirStl%\%inputFolder%\*.%inputMeshFormat%) do (echo "%%X")
...
rem Run the "for" loop from inside the input folder
cd %dirStl%\%inputFolder%

for %%I in (*.%inputMeshFormat%) do (%meshlabserverPath% -i %%I -o ..\..\%dirObj%\%outputFolder%\%%~nI.%outputMeshFormat% -s ..\..\%mlxScriptFolder%\%mlxScriptFile% %outputMeshOptions%)
rem To get help on the "for" syntax use: for /?
...
echo %dirObj%\%outputFolder%
for %%X in (..\%dirObj%\%outputFolder%\*.*) do (echo "%%X")
rem To get help on the "for" syntax use: for /?
echo.
